...
cWAS <- function(gene_df_f,weight_f,r2_f,
  ...
  if(!file.exists(paste0(ref_geno_f,".bim"))){
    ref_geno <- fread(paste0(ref_geno_f,".bim.gz"),header=F,stringsAsFactors=F,sep="\t")
  }else{
    ref_geno <- fread(paste0(ref_geno_f,".bim"),header=F,stringsAsFactors=F,sep="\t")
  }
...
#  geno <- geno[ind_match,]
  ...
  gwas_beta <- gwas_beta[match(inter_iden,bim_iden),]
  geno_se <- apply(geno, 1, function(X) sd(na.omit(X)))
  geno_mean <- apply(geno, 1, function(X) mean(na.omit(X)))
