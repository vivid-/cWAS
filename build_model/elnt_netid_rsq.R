...
weight_file <- paste0("/ysm-gpfs/scratch60/",netid,"/GTEx/v8/models/",chr,"/",gene,"/",tissue,"_weight.txt")
if(!file.exists(weight_file)){
  next()
}
cat(weight_file,"\n")
weight <- read.table(weight_file,header=T,stringsAsFactors=F,sep="\t")
w <- rep(0,dim(bim)[1])
w[match(weight[,2],bim[,2])] <- weight$weight
pred.mat <- t(bed_1)%*%as.matrix(w,ncol=1)
