...
load("/gpfs/ysm/pi/zhao-data/wl382/cWAS/data/GTEX/v6/WB_sel_predExpr_elnt.Rdata")
sel <- read.table("genes_mixed_sel_WB.txt",header=T,stringsAsFactors=F,sep="\t")
result <- data.frame()
...
write.table(result,file="summary_WB_pred_v8_v6_mixed_sel.txt",col.names=T,row.names=F,sep="\t",quote=F)
