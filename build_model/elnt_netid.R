...
fit <- cv.glmnet(scale(t(bed_1),scale=F,center=F),scale(expr_1[,2],center=T,scale=F),nfolds=k,alpha=alpha,keep=T,parallel=F) 
...
write.table(result,file=paste0("/ysm-gpfs/scratch60/",netid,"/GTEx/v8/models/",chr,"/",gene,"/",tissue,"_weight.txt"),col.names=T,row.names=F,sep="\t",quote=F)
...
write.table(result_df,file=paste0("/ysm-gpfs/scratch60/",netid,"/GTEx/v8/models/",chr,"/",gene,"/",tissue,"_rsq.txt"),col.names=T,row.names=F,sep="\t",quote=F)
